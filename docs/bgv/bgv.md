...
If $x \in \ZZ_m$, then we define ${[}\cdot {]}$ as a map from
$\ZZ_m$ to $\ZZ$ as

$$
\begin{aligned}
{[}\cdot{]} &: \ZZ_m \rightarrow \ZZ \\
[x] &= \{ x + m\ZZ \} \cap [m].
\end{aligned}
$$

In other words, $[x]$ is **an integer** in the range $0$ to $m-1$, such
that $[x]\,\text{mod}\, m$ lies in the same residue class as $x.$
...
$$
[11 + 7\ZZ] = \{ 11, 11 \pm 7, 11 \pm 14, \cdots \} \cap
  \{0,1,\cdots, 6\} = 4 \in \ZZ.
$$
...
extend the definition of $[\cdot]$ to polynomials as

$$
\begin{aligned}
  {[}\cdot{]} &: (\Zmod{q})[X] \rightarrow \ZZ[X]\\
  \left [\sum a_iX^i \right] &= \sum [a_i]X^i
\end{aligned}
$$
...
$$
\begin{aligned}
\dec(\vec{s}, (\vec{a}, \vec{b})) = m &:= \overbrace{\left[\underbrace{[\vec{b}
- \vec{a}\cdot\vec{s}]}_{\in R}\,(\text{mod}\,q)\right]}^{\in R}\;(\text{mod}\,
p) & \in [p]
\end{aligned}
$$

Since $p|\vec{e}| \ll q$ with overwhelming probability, $|p\vec{e} +
m| \leq |p\vec{e}| + p \ll q$. Therefore, $[\vec{c} -
\vec{a}\cdot\vec{s}]\,(\text{mod}\,q) = p\vec{e} + m$,
and correctness of decryption follows immediately.
...
??? example
    ...
    $$
      \begin{aligned}
        ...
        [\vec{a}\cdot \vec{s}] &= 700Y^3 + 744Y^2 + 958Y + 988 \in R \\
        [\vec{b}] &= [\vec{a}\cdot \vec{s}] + p\vec{e} + m \in R      \\
            &= (700Y^3 + 744Y^2 + 958Y + 988) + 8(4Y^3 + 2Y^2 - 3Y) + 5 \\
        \vec{b} &= [\vec{b}]\;\text{mod}\, q \\
    ...
    $$
      \begin{aligned}
        ...
        [\vec{b} - \vec{a}\cdot\vec{s}] &= 32Y^3 + 16Y^2 + 1000Y + 5 \in R \\
        \dec(\vec{c}) &= [\vec{b} - \vec{a}\cdot\vec{s}]\;(\text{mod}\, p) \in [p] \\
                      &= 32Y^3 + 16Y^2 + 1000Y + 5\;(\text{mod}\, 8) \in [p]  \\
                      &= 8(4Y^3 + 2Y^2 + 125Y) + 5\;(\text{mod}\, 8) \in [p] \\
                      &= 5
      \end{aligned}
    $$
...
it to $R := \ZZ[X]/\langle \Phi_m \rangle$. Usually a modulo
reduction from $\ZZ \mapsto \ZZ_q$ followed by a lift from $\ZZ_q
\mapsto \ZZ$ is not an identity operation, however, since $pB \ll
q$, $[\vec{b} -\vec{a}\cdot\vec{s}] = [p\cdot \vec{e} + m] \in
\ZZ[X]/\langle \Phi_m \rangle$ has _exactly the same coefficients_
as $p\cdot \vec{e} + m \in \ZZ[X]/\langle \Phi_m, q \rangle$.

This means that from $p\cdot\vec{e} + m \in \ZZ[X]/\langle \Phi_m,
q\rangle$ one can compute $\left([p\cdot\vec{e} +
m]\right)\;\text{mod}\, p = [m] \;\text{mod}\, p \in \ZZ[X]/\langle
\Phi_m, p\rangle$ and treat it as an element of $R_p :=
\ZZ[X]/\langle \Phi_m, p\rangle$. As we will later see, this allows
for embedding multiple messages in a single ciphertext and enables
...
scheme. We summarize the entire encryption scheme below:
